...
### SXM_searchFiles
**Search files with filters** according to some properties - filename, real image size, setpoint, scantime, comments - through one or multiple folders. 
<br>Useful e.g. when scraping through old data.
...
It's designed for Nanonis .SXM files/metadata (also when saved as .gwy), but **can be adapted** to other formats (I'd like to make it a 'modular' search script, but haven't had the need so far: please get in contact if interested)

The first part of the file contains options/customizations with comments that should make them understandable:
```python
### Folder and filetype ###
...
### export_profiles
My very first script: export all the profiles (Graphs) in the open files to ASCII .txt files. Intended for use in some other program (IgorPro or similar).

It works either recursively on all open files or only on the active one. The profiles are exported as OriginaleFilename_prof_xx.txt


# Scripts repository
These are scripts contributed by other authors + some that I've collected for ease of use - mostly the result of a 'pygwy' search in GitHub discarding obsolete projects. 
<br> If you want your script added to (or removed from) this list, feel free to contact me - I have no intention to steal anyone's credits.

Note: they are **untested** unless otherwise stated

### Contributed
- [MohamedAzizAllani/AutoProcess_plugin-Gwyddion](https://github.com/MohamedAzizAllani/AutoProcess_plugin-Gwyddion): a great GUI plugin to apply several operations (color gradient and range, crop, renaming, or even custom macros) to a series of files or channels to be selected. 
<br> Tested and working on Gwyddion 2.69
- [rohanplatts/Python-in-Gwyddion](https://github.com/rohanplatts/Python-in-Gwyddion): not really PyGwy, but very useful: a Gwyddion module that executes any python script from any python environment (e.g. installed with conda/anaconda: so also python 3) in Gwyddion. The script is executed on the open image and reads the output back in Gwyddion. It's compiled for Windows 64bit only, which means it can't work with PyGwy (32bit only) but also having some python capabilities in 64bit installations
- [rohanplatts/nanonis-file-conversion](https://github.com/rohanplatts/nanonis-file-conversion): also not really PyGwy: scripts to convert Nanonis human-readable .dat files to both .sxm (Gwyddion-readable) and .jpg
...
- [Drilack7/Python-Scripts-for-Gwyddion](https://github.com/Drilack7/Python-Scripts-for-Gwyddion): different scripts for
  ...
  - simple batch operations (change Color scale, Scale min/max, Level)
...
- [wampiter/gwyscripts](https://github.com/wampiter/gwyscripts): batch image processing scripts to
  ...
  - save all open files to a folder with corresponding .png
